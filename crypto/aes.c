/*
 * aes.c — AES-128 Implementation
 *
 * Pure C implementation of AES-128, ECB and CBC modes.
 * No external dependencies. No heap allocation.
 * Suitable for ARM Cortex-M embedded targets.
 *
 * References:
 *   FIPS 197 — Advanced Encryption Standard (AES)
 *   https://nvlpubs.nist.gov/nistpubs/FIPS/NIST.FIPS.197.pdf
 */

#include "aes.h"
#include <string.h>

/* ------------------------------------------------------------------ */
/* AES S-Box and Inverse S-Box (FIPS 197, Figure 7)                   */
/* ------------------------------------------------------------------ */

static const uint8_t SBOX[256] = {
    
};

static const uint8_t INV_SBOX[256] = {
    
};

/* Round constants for key schedule (FIPS 197, Section 5.2) */
static const uint8_t RCON[11] = {
    0x00,0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80,0x1b,0x36
};

/* ------------------------------------------------------------------ */
/* Internal helpers                                                    */
/* ------------------------------------------------------------------ */

/* Galois Field multiplication by 2 (xtime) */
static inline uint8_t xtime(uint8_t x) {
    return (x << 1) ^ ((x & 0x80) ? 0x1b : 0x00);
}

/* GF(2^8) multiply */
static inline uint8_t gf_mul(uint8_t a, uint8_t b) {
    uint8_t result = 0;
    while (b) {
        if (b & 1) result ^= a;
        a = xtime(a);
        b >>= 1;
    }
    return result;
}

/* ------------------------------------------------------------------ */
/* Key Schedule — expand 16-byte key into 11 round keys               */
/* ------------------------------------------------------------------ */

void AES_init(AES_ctx *ctx, const uint8_t key[AES_KEY_SIZE]) {
    uint8_t *rk = ctx->round_keys[0];

    /* Copy original key as round key 0 */
    memcpy(rk, key, AES_KEY_SIZE);

    for (int i = 1; i <= AES_NUM_ROUNDS; i++) {
        uint8_t *prev = ctx->round_keys[i - 1];
        uint8_t *curr = ctx->round_keys[i];

        /* RotWord + SubWord + Rcon on last word of previous round key */
        curr[0]  = SBOX[prev[13]] ^ RCON[i] ^ prev[0];
        curr[1]  = SBOX[prev[14]] ^ prev[1];
        curr[2]  = SBOX[prev[15]] ^ prev[2];
        curr[3]  = SBOX[prev[12]] ^ prev[3];

        /* Remaining 3 words = XOR with previous round key */
        for (int j = 4; j < 16; j++)
            curr[j] = curr[j - 4] ^ prev[j];
    }
}

/* ------------------------------------------------------------------ */
/* AES Round Operations                                                */
/* ------------------------------------------------------------------ */

static void sub_bytes(uint8_t state[16]) {
    for (int i = 0; i < 16; i++)
        state[i] = SBOX[state[i]];
}

static void inv_sub_bytes(uint8_t state[16]) {
    for (int i = 0; i < 16; i++)
        state[i] = INV_SBOX[state[i]];
}

static void shift_rows(uint8_t state[16]) {
    uint8_t tmp;
    /* Row 1: shift left by 1 */
    tmp=state[1]; state[1]=state[5]; state[5]=state[9]; state[9]=state[13]; state[13]=tmp;
    /* Row 2: shift left by 2 */
    tmp=state[2]; state[2]=state[10]; state[10]=tmp;
    tmp=state[6]; state[6]=state[14]; state[14]=tmp;
    /* Row 3: shift left by 3 (= shift right by 1) */
    tmp=state[15]; state[15]=state[11]; state[11]=state[7]; state[7]=state[3]; state[3]=tmp;
}

static void inv_shift_rows(uint8_t state[16]) {
    uint8_t tmp;
    /* Row 1: shift right by 1 */
    tmp=state[13]; state[13]=state[9]; state[9]=state[5]; state[5]=state[1]; state[1]=tmp;
    /* Row 2: shift right by 2 */
    tmp=state[2]; state[2]=state[10]; state[10]=tmp;
    tmp=state[6]; state[6]=state[14]; state[14]=tmp;
    /* Row 3: shift right by 3 (= shift left by 1) */
    tmp=state[3]; state[3]=state[7]; state[7]=state[11]; state[11]=state[15]; state[15]=tmp;
}

static void mix_columns(uint8_t state[16]) {
    for (int i = 0; i < 4; i++) {
        uint8_t *col = &state[i * 4];
        uint8_t s0=col[0], s1=col[1], s2=col[2], s3=col[3];
        col[0] = gf_mul(s0,2) ^ gf_mul(s1,3) ^ s2 ^ s3;
        col[1] = s0 ^ gf_mul(s1,2) ^ gf_mul(s2,3) ^ s3;
        col[2] = s0 ^ s1 ^ gf_mul(s2,2) ^ gf_mul(s3,3);
        col[3] = gf_mul(s0,3) ^ s1 ^ s2 ^ gf_mul(s3,2);
    }
}

static void inv_mix_columns(uint8_t state[16]) {
    for (int i = 0; i < 4; i++) {
        uint8_t *col = &state[i * 4];
        uint8_t s0=col[0], s1=col[1], s2=col[2], s3=col[3];
        col[0] = gf_mul(s0,0x0e)^gf_mul(s1,0x0b)^gf_mul(s2,0x0d)^gf_mul(s3,0x09);
        col[1] = gf_mul(s0,0x09)^gf_mul(s1,0x0e)^gf_mul(s2,0x0b)^gf_mul(s3,0x0d);
        col[2] = gf_mul(s0,0x0d)^gf_mul(s1,0x09)^gf_mul(s2,0x0e)^gf_mul(s3,0x0b);
        col[3] = gf_mul(s0,0x0b)^gf_mul(s1,0x0d)^gf_mul(s2,0x09)^gf_mul(s3,0x0e);
    }
}

static void add_round_key(uint8_t state[16], const uint8_t round_key[16]) {
    for (int i = 0; i < 16; i++)
        state[i] ^= round_key[i];
}

/* ------------------------------------------------------------------ */
/* AES ECB Encrypt / Decrypt                                           */
/* ------------------------------------------------------------------ */

void AES_ECB_encrypt(const AES_ctx *ctx, uint8_t block[AES_BLOCK_SIZE]) {
    uint8_t state[16];
    memcpy(state, block, 16);

    add_round_key(state, ctx->round_keys[0]);

    for (int round = 1; round <= AES_NUM_ROUNDS; round++) {
        sub_bytes(state);
        shift_rows(state);
        if (round < AES_NUM_ROUNDS)
            mix_columns(state);
        add_round_key(state, ctx->round_keys[round]);
    }

    memcpy(block, state, 16);
}

void AES_ECB_decrypt(const AES_ctx *ctx, uint8_t block[AES_BLOCK_SIZE]) {
    uint8_t state[16];
    memcpy(state, block, 16);

    add_round_key(state, ctx->round_keys[AES_NUM_ROUNDS]);

    for (int round = AES_NUM_ROUNDS - 1; round >= 0; round--) {
        inv_shift_rows(state);
        inv_sub_bytes(state);
        add_round_key(state, ctx->round_keys[round]);
        if (round > 0)
            inv_mix_columns(state);
    }

    memcpy(block, state, 16);
}

/* ------------------------------------------------------------------ */
/* AES CBC Encrypt / Decrypt                                           */
/* ------------------------------------------------------------------ */

void AES_CBC_encrypt(const AES_ctx *ctx,
                     uint8_t iv[AES_BLOCK_SIZE],
                     uint8_t *buffer,
                     size_t length) {
    for (size_t i = 0; i < length; i += AES_BLOCK_SIZE) {
        /* XOR plaintext block with IV (or previous ciphertext block) */
        for (int j = 0; j < AES_BLOCK_SIZE; j++)
            buffer[i + j] ^= iv[j];
        AES_ECB_encrypt(ctx, &buffer[i]);
        /* Update IV to current ciphertext block */
        memcpy(iv, &buffer[i], AES_BLOCK_SIZE);
    }
}

void AES_CBC_decrypt(const AES_ctx *ctx,
                     uint8_t iv[AES_BLOCK_SIZE],
                     uint8_t *buffer,
                     size_t length) {
    uint8_t next_iv[AES_BLOCK_SIZE];
    for (size_t i = 0; i < length; i += AES_BLOCK_SIZE) {
        memcpy(next_iv, &buffer[i], AES_BLOCK_SIZE);
        AES_ECB_decrypt(ctx, &buffer[i]);
        /* XOR with previous IV */
        for (int j = 0; j < AES_BLOCK_SIZE; j++)
            buffer[i + j] ^= iv[j];
        memcpy(iv, next_iv, AES_BLOCK_SIZE);
    }
}